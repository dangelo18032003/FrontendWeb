  ...
  useEffect(() => {
    if (id) {
      const fetchProduct = async () => {
        try {
          const response = await axios.get(`https://productsservice-gwhp.onrender.com/${id}`, {
            headers: {
...
const handleImageUpload = async (event) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = id
        ? `https://productsservice-gwhp.onrender.com/product/${id}`
        : 'https://productsservice-gwhp.onrender.com/product/create';
