...
const Home = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        setIsLoading(true);
        const response = await fetch('https://authenticationservice-gq22.onrender.com/profile', {
          headers: {
  ...
  const fetchCart = async () => {
    ...
    try {
      setIsLoading(true);
      const response = await fetch('https://authenticationservice-gq22.onrender.com/cart', {
        headers: {
          'Authorization': `Bearer ${authState.token}`,
          'Content-Type': 'application/json',
        },
