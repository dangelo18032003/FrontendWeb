  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const profileResponse = await fetch(`https://authenticationservice-gq22.onrender.com/profile`, {
          method: 'GET',
          headers: {
        ...
        const wishlistDetails = await Promise.all(
          wishlistIds.map(async (id) => {
            ...
            const itemResponse = await fetch(`https://authenticationservice-gq22.onrender.com/${id}`, {
              method: 'GET',
              headers: {
