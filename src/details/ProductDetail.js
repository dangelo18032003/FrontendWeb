...
const ProductDetail = () => {
  ...
  useEffect(() => {
    const fetchProductAndSellerDetails = async () => {
      try {
        const productResponse = await fetch(`https://productsservice-gwhp.onrender.com/${id}`, {
        ...
        if (productData.seller) {
          try {
            const sellerResponse = await fetch(`https://authenticationservice-gq22.onrender.com/product/seller/${productData.seller}`, {
              headers: { 'Authorization': `Bearer ${authState.token}` },
            });
